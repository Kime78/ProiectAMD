...
void ProductDatabase::add_product(Product prod) { //to be worked on :(
    ...
    case ProductType::GPU:
        products << prod.cpu.get_weight() << "\t" << prod.cpu.get_height() << "\t" << prod.cpu.get_nms() << "\t" << prod.cpu.get_tdp() << "\t" << prod.cpu.get_memory() << "\t" << prod.cpu.get_clock() << "\tGPU\t";
    ...
    case ProductType::APU:
        products << prod.cpu.get_weight() << "\t" << prod.cpu.get_height() << "\t" << prod.cpu.get_nms() << "\t" << prod.cpu.get_tdp() << "\t" << prod.cpu.get_memory() << "\t" << prod.cpu.get_clock() << "\tAPU\t";
        products << prod.cpu.get_cores() << "\t" << prod.cpu.get_threads() << "\t" << prod.cpu.get_socket() << "\t";
        products << prod.gpu.get_max_horizontal_res() << "\t" << prod.gpu.get_max_vertical_res() << "\t";
        switch (prod.gpu.get_supported_tech()) {
        case SupportedTechnologies::OpenGL:
...
void ProductDatabase::load_products_from_file() { //to be worked on :(
    ...
    while(product_file >> id >> name >> price >> weight >> height >> nms >> tdp >> memory >> clock >> type) {
        ...
        if(type == "GPU") {
            ...
            prod->gpu = GPU(max_vertical_res, max_horizontal_res, s_tech);
...
void ProductDatabase::remove_product(uint64_t id) {
    ...
    if(prod.type == ProductType::CPU) {
        ...
        delete_line = std::to_string(prod.cpu.get_weight()) + "\t" + std::to_string(prod.cpu.get_height()) + "\t" + std::to_string(prod.cpu.get_nms()) + "\t" + std::to_string(prod.cpu.get_tdp()) + "\t" + std::to_string(prod.cpu.get_memory()) + "\t" + std::to_string(prod.cpu.get_clock()) + "\tGPU\t";
